...
>(({ className, children, ...props }, ref) => (
  <DialogoPortal>
    ...
    <DialogoPrimitivo.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 data-[state=open]:zoom-in-95 data-[state=closed]:zoom-out-95 rounded-2xl",
        className
      )}
      {...props}
    >
      {children}
      <DialogoPrimitivo.Close className="absolute right-4 top-4 rounded-[8px] opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
        <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <DialogoPrimitivo.Title
    ref={ref}
    className={cn("font-titulo text-lg font-semibold leading-none tracking-tight", className)}
    {...props}
  />
));
